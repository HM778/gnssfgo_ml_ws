...
namespace gnss_comm
{
    ...
    * 卫星编号范围	对应系统	系统内PRN范围	说明
        1 - 32	    GPS	        1 - 32	前32个编号分配给GPS
        33 - 59	    GLONASS	    1 - 27	编号33-59分配给GLONASS
        60 - 97	    Galileo	    1 - 38	编号60-97分配给Galileo
        98 - 160	BeiDou	    1 - 63	编号98-160分配给BeiDou
    *-----------------------------------------------------------------------------*/
    uint32_t satsys(uint32_t sat, uint32_t *prn)
    {
        ...
        else if ((sat-=N_SAT_GPS) <= N_SAT_GLO && (sat-=N_SAT_GPS)>0) {
            sys = SYS_GLO; sat += MIN_PRN_GLO-1;
        }
        else if ((sat-=N_SAT_GLO) <= N_SAT_GAL && (sat-=N_SAT_GLO)>0) {
            sys = SYS_GAL; sat += MIN_PRN_GAL-1;
        }
        else if ((sat-=N_SAT_GAL) <= N_SAT_BDS && (sat-=N_SAT_GAL)>0) {
    ...
    *-----------------------------------------------------------------------------*/
    ...
    Eigen::Vector3d eph2pos(const gtime_t &curr_time, const EphemPtr ephem_ptr, double *svdt)
    {
        ...
        if (std::abs(tk) > EPH_VALID_SECONDS)
            LOG(WARNING) << "Ephemeris is not valid anymore : " << ephem_ptr->sat << "->time out:" << std::abs(tk);
        
        uint32_t prn = ephem_ptr->sat;
        ...
        double dt = time_diff(curr_time, ephem_ptr->toc); //信号发射时间
    ...
    *-----------------------------------------------------------------------------*/
    ...
    double L2_freq(const ObsPtr &obs, int *l2_idx)
    {
        ...
        double freq_max = -1.0;
        if (sys == SYS_GPS)
            freq_min = freq_max = FREQ2;                        // L2: 1.22760E9
        else if (sys == SYS_GAL)
            freq_min = freq_max = FREQ7;                        // E5b: 1.20714E9
        else if (sys == SYS_BDS)
            freq_min = freq_max = FREQ2_BDS;                    // B2: 1.20714E9
        else if (sys == SYS_GLO)
        {
    ...
    bool has_dual_freq(const ObsPtr &obs)
    {
        ...
        L1_freq(obs, &l1_idx);
        L2_freq(obs, &l2_idx);
        return (l1_idx >= 0 && l2_idx >= 0);
    }

    bool get_L1_L2_freqs(const ObsPtr &obs, double &f1, double &f2)
    {
        int l1_idx = -1, l2_idx = -1;
        f1 = L1_freq(obs, &l1_idx);
        f2 = L2_freq(obs, &l2_idx);
        return (l1_idx >= 0 && l2_idx >= 0 && f1 > 0.0 && f2 > 0.0);
    }

    double psr_iono_free(double f1, double f2, double P1, double P2)
    {
        const double f1_sq = f1 * f1;
        const double f2_sq = f2 * f2;
        const double denom = f1_sq - f2_sq;
        if (std::abs(denom) < 1e-6)
            return 0.0;
        return (f1_sq * P1 - f2_sq * P2) / denom;
    }
    ...
    double cp_iono_free(double f1, double f2, double L1, double L2)
    {
        ...
        if (std::abs(denom) < 1e-6)
            return 0.0;
        return (f1_sq * L1 - f2_sq * L2) / denom;
    }
    ...
    double dopp_iono_free(double f1, double f2, double D1, double D2)
    {
        ...
        if (std::abs(denom) < 1e-6)
            return 0.0;
        return (f1_sq * D1 - f2_sq * D2) / denom;
    }
    ...
    double if_noise_factor(double f1, double f2)
    {
        ...
        if (std::abs(denom) < 1e-6)
            return 1.0;
        const double alpha = f1_sq / denom;
        const double beta  = f2_sq / denom;
