...
namespace gnss_comm
{
    ...
    *-----------------------------------------------------------------------------*/
    ...
    /* Get L2 frequency  ---------------------------------------------------------
    * args   : ObsPtr&             obs      I   GNSS observation
    *          int*                l2_idx   I/O L2 frequency index (NULL if not required)
    * return : L2 frequency (negative if L2 frequency not exists)
    *-----------------------------------------------------------------------------*/
    double L2_freq(const ObsPtr &obs, int *l2_idx);

    /* Check if satellite has dual-frequency (L1+L2) observations -------------------
    * args   : ObsPtr&             obs      I   GNSS observation
    * return : true if both L1 and L2 frequencies are available
    *-----------------------------------------------------------------------------*/
    bool has_dual_freq(const ObsPtr &obs);

    /* Get L1 and L2 frequencies for a satellite ------------------------------------
    * args   : ObsPtr&             obs      I   GNSS observation
    *          double&             f1       O   L1 frequency (Hz)
    *          double&             f2       O   L2 frequency (Hz)
    * return : true if both frequencies are available
    *-----------------------------------------------------------------------------*/
    bool get_L1_L2_freqs(const ObsPtr &obs, double &f1, double &f2);

    /* Ionosphere-Free pseudorange combination --------------------------------------
    * P_IF = (f1^2 * P1 - f2^2 * P2) / (f1^2 - f2^2)
    * args   : double f1       I   L1 frequency (Hz)
    *          double f2       I   L2 frequency (Hz)
    *          double P1       I   L1 pseudorange (m)
    *          double P2       I   L2 pseudorange (m)
    * return : ionosphere-free pseudorange (m)
    *-----------------------------------------------------------------------------*/
    double psr_iono_free(double f1, double f2, double P1, double P2);

    /* Ionosphere-Free carrier-phase combination ------------------------------------
    * L_IF = (f1^2 * L1 - f2^2 * L2) / (f1^2 - f2^2)  (unit: cycles)
    * args   : double f1       I   L1 frequency (Hz)
    *          double f2       I   L2 frequency (Hz)
    *          double L1       I   L1 carrier-phase (cycles)
    *          double L2       I   L2 carrier-phase (cycles)
    * return : ionosphere-free carrier-phase (cycles)
    *-----------------------------------------------------------------------------*/
    double cp_iono_free(double f1, double f2, double L1, double L2);

    /* Ionosphere-Free Doppler combination ------------------------------------------
    * D_IF = (f1^2 * D1 - f2^2 * D2) / (f1^2 - f2^2)
    * args   : double f1       I   L1 frequency (Hz)
    *          double f2       I   L2 frequency (Hz)
    *          double D1       I   L1 Doppler (Hz)
    *          double D2       I   L2 Doppler (Hz)
    * return : ionosphere-free Doppler (Hz)
    *-----------------------------------------------------------------------------*/
    double dopp_iono_free(double f1, double f2, double D1, double D2);

    /* Ionosphere-Free combination noise amplification factor -----------------------
    * sigma_IF = sqrt(alpha^2 + beta^2) * sigma_L1, where
    * alpha = f1^2 / (f1^2 - f2^2), beta = f2^2 / (f1^2 - f2^2)
    * args   : double f1       I   L1 frequency (Hz)
    *          double f2       I   L2 frequency (Hz)
    * return : noise amplification factor (>= 1.0, typically ~3.0)
    *-----------------------------------------------------------------------------*/
    double if_noise_factor(double f1, double f2);
