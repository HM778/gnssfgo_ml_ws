...
if [ "${ENABLE_OSQA}" = "1" ]; then
    # 可选：开启 Transformer/OSQA（传参 1）
    echo "Running GNSS Quality Analyzer..."
    gnome-terminal -- bash -c "cd \"${WS_DIR}/GNSS-Transformer/gnss_quality_analyzer\" || exit 1
python3 run_analyzer.py --gnssfgo-input \"${OSQA_INPUT_PATH}\" --gnssfgo-output \"${OSQA_OUTPUT_PATH}\" --urban; exec bash"
    sleep 3s
else
